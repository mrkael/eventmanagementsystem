...
<body style="margin:0;background:#f8fafc;font-family:Arial,sans-serif;color:#0f172a;">
    <div style="max-width:640px;margin:0 auto;padding:28px 16px;">
        <div style="background:#ffffff;border:1px solid #e2e8f0;border-radius:12px;padding:24px;">
            @if($renderedHeader)
                <div style="margin:0 0 18px;font-size:22px;font-weight:bold;line-height:1.3;">{!! $renderedHeader !!}</div>
            @elseif($registration)
            ...
            @if($renderedFooter)
                <div style="margin-top:24px;border-top:1px solid #e2e8f0;padding-top:16px;font-size:13px;line-height:1.6;color:#64748b;">{!! $renderedFooter !!}</div>
            @endif
        </div>
    </div>
</body>
