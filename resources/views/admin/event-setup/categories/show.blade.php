<x-layouts.admin title="Event Category Detail" heading="{{ $category->name }}" subheading="Category record">
    <section class="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div class="mt-6 flex gap-3"><a href="{{ route('admin.event-categories.edit', $category) }}" class="inline-flex min-h-11 items-center rounded-lg bg-emerald-700 px-4 text-sm font-semibold text-white hover:bg-emerald-800">Edit</a><a href="{{ route('admin.event-categories.index') }}" class="inline-flex min-h-11 items-center rounded-lg border border-slate-300 px-4 text-sm font-semibold hover:bg-slate-50">Back</a></div>
    </section>
</x-layouts.admin>
